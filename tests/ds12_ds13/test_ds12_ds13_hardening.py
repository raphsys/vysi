...
def test_all_native_profiles_reach_quality_complete(
    ...
    assert mapping["header"]["schema_version"] == "2.8.0"
    assert coverage["header"]["schema_version"] == "2.8.0"
    assert preservation["header"]["schema_version"] == "2.8.0"
...
def test_mapping_edges_are_canonical_deterministic_and_evidenced(tmp_path: Path) -> None:
    ...
    for edge in mapping["mappings"]:
        ...
        assert edge["producer_version"] == "0.8.1"
    ...
    validate_mapping_invariants(mapping, producer_version="0.8.1")
...
def test_required_rendering_is_measured_as_declared_technical_preview(tmp_path: Path) -> None:
    ...
    feature = next(item for item in coverage["items"] if item["feature"] == "rendered_surface")
    assert feature["required"] == 1
    assert feature["rendered"] == 1
    assert feature["approximated"] == 1
    assert feature["omitted"] == 0
    assert coverage["axis_scores"]["visual"] == 0.5
    assert coverage["header"]["status"] == "review"
    transition = next(
        item
        for item in preservation["transitions"]
        if item["from_layer"] == "native" and item["to_layer"] == "rendered"
    )
    assert transition["state"] == "partial"
    assert transition["exactness"] == "approximate"
