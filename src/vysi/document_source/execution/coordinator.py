...
def _write_checkpoint(
    ctx: ExecutionContext, request_hash: str, *, target_stage: str, complete: bool
) -> None:
    ...
    if complete and target_stage in {"DS06", "DS07", "DS08", "DS09", "DS10", "DS12", "DS13"}:
        (ctx.workspace / "PREFLIGHT_COMPLETE").write_text("DS00-DS06\n", encoding="utf-8")
    if complete and target_stage in {"DS09", "DS10", "DS12", "DS13"}:
        (ctx.workspace / "NATIVE_COMPLETE").write_text("DS00-DS09\n", encoding="utf-8")
    if complete and target_stage in {"DS10", "DS12", "DS13"}:
        (ctx.workspace / "IR_COMPLETE").write_text("DS00-DS10\n", encoding="utf-8")
    if complete and target_stage in {"DS12", "DS13"}:
        (ctx.workspace / "MAPPING_COMPLETE").write_text("DS00-DS10,DS12\n", encoding="utf-8")
    if complete and target_stage == "DS13":
...
def _run(
    ...
    *,
    ...
    if resume_from is not None:
        ...
        if (resume_from / target_marker).is_file() and set(stages).issubset(completed):
            return PipelineResult(
                resume_from, ctx.run_id, "succeeded", tuple(sorted(completed | {"DS00"}))
            )
    ...
    try:
        ...
        _write_checkpoint(ctx, request_hash, target_stage=target_stage, complete=True)
        result_state = "partial" if final_status == "review" else "succeeded"
        return PipelineResult(
