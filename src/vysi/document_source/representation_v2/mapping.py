...
def build_mapping_catalog(
    *,
    ...
    if rendered is not None:
        ir_by_native: dict[str, set[str]] = defaultdict(set)
        for unit in ir_units:
            unit_id = str(unit["unit_id"])
            for native_id in unit.get("source_native_ids", []):
                ir_by_native[str(native_id)].add(unit_id)
        for surface in cast(list[dict[str, Any]], rendered.get("surfaces", [])):
            surface_id = str(surface["surface_id"])
            sources = tuple(sorted(set(str(item) for item in surface.get("native_unit_refs", []))))
            ...
            if sources:
                ...
                if fidelity == "exact":
                    ...
                    confidence = 0.7
                    method = "rendered_surface_visible_native_refs"
                else:
                    exactness = "inferred"
                    confidence = 0.5
                    method = "rendered_surface_passthrough_reference_not_visually_assessed"
            ...
            ir_sources = tuple(sorted({unit for source in sources for unit in ir_by_native.get(source, set())}))
            if not ir_sources and synthetic_roots:
                ir_sources = synthetic_roots
            if ir_sources:
                edges.append(
                    _edge(
                        document_id=document_id,
                        producer_version=producer_version,
                        source_layer="technical_ir",
                        source_ids=ir_sources,
                        target_layer="rendered",
                        target_ids=[surface_id],
                        relation="represented_on_surface",
                        exactness=exactness,
                        confidence=confidence,
                        method="rendered_surface_ir_provenance",
                        evidence_refs=evidence,
                    )
                )
    ...
    if geometry is not None:
        ...
        for item in cast(list[dict[str, Any]], geometry.get("geometries", [])):
            ...
            native_refs = tuple(sorted(set(str(ref) for ref in item.get("native_unit_refs", []))))
            if native_refs:
                unknown_native = set(native_refs) - set(inventory.ids("native"))
                if unknown_native:
                    raise MappingError(
                        f"Géométrie {item['geometry_id']} référence des unités natives inconnues: "
                        f"{sorted(unknown_native)}"
                    )
                visibility = str(item.get("visibility", "not_assessed"))
                geometry_exactness = cast(Any, "approximate" if visibility == "partially_clipped" else "exact")
                geometry_confidence = 0.7 if visibility == "partially_clipped" else 1.0
                edges.append(
                    _edge(
                        document_id=document_id,
                        producer_version=producer_version,
                        source_layer="native",
                        source_ids=native_refs,
                        target_layer="geometry",
                        target_ids=[str(item["geometry_id"])],
                        relation="localized_in_geometry",
                        exactness=geometry_exactness,
                        confidence=geometry_confidence,
                        method="rendered_element_native_refs",
                        evidence_refs=evidence,
                    )
                )
