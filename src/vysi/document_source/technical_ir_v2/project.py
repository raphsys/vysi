...
def _prefixed_native_property(prefix: str, item: dict[str, Any]) -> dict[str, Any]:
    return {
        "name": canonical_property_name(f"{prefix}.{item['name']}"),
...
def _append_styles(
    ...
    ordinal = len(units)
    for offset, style in enumerate(styles["styles"]):
        native_id = str(style["style_id"])
        address = str(style["source_address"])
        direct = [
            _prefixed_native_property("style.direct", item)
            for item in style.get("direct_properties", [])
        ]
        resolved = [
            _prefixed_native_property("style.resolved", item)
            for item in style.get("resolved_properties", [])
        ]
        units.append(
            _unit(
                unit_id=_ir_id(document_id, native_id, "style_definition"),
                ...
                protected=True,
                technical_properties=[
                    _property("style.kind", style["style_kind"], address=address),
                    _property("style.native_name", style.get("native_name"), address=address),
                    _property(
                        "style.parent_style_id", style.get("parent_style_id"), address=address
                    ),
                    _property("source.address", address, address=address),
                    *direct,
                    *resolved,
...
def _append_metadata(
    ...
    ordinal = len(units)
    for offset, item in enumerate(metadata["items"]):
        native_id = str(item["metadata_id"])
        address = str(item["source_address"])
        units.append(
            _unit(
                unit_id=_ir_id(document_id, native_id, "metadata"),
                kind="metadata",
                parent_id=root_id,
                ordinal=ordinal + offset,
                origin="native_projection",
...
def project_document(
    ...
    for catalog in (styles, relationships, metadata, annotations, resources):
    ...
    _append_styles(units, document_id, root_id, styles)
    _append_metadata(units, document_id, root_id, metadata)
