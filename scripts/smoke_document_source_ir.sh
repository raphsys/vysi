...
SOURCE="$ROOT/runtime/tmp/ir_${STAMP}.txt"
OUTPUT="$ROOT/runtime/results/document_source"
mkdir -p "$(dirname "$SOURCE")" "$OUTPUT"
printf 'Vysi DS00-DS10\nProjection IR déterministe.\n' > "$SOURCE"
CHECKPOINT="$($PY -m vysi.document_source.cli ir --source "$SOURCE" --output "$OUTPUT")"
...
find "$CHECKPOINT/ir" -name technical_document_ir.json -print -quit | grep -q . || { echo "TechnicalDocumentIR absent" >&2; exit 1; }
PYTHONPATH="$ROOT/src" "$PY" "$ROOT/tools/validate_preflight_checkpoint.py" "$CHECKPOINT"
echo "Smoke DS00-DS10 réussi : $CHECKPOINT"
